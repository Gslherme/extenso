...
//Função main com a interface do usuário
int main(void) {
    long long int num;
    int continuar;
    ...
    do {
        system("cls");
        printf("Programa Que Escreve Por Extenso - Gslherme \n");
        ...
        bilhao_trilhao(num); //Envia o número para ser processado por uma cadeia de funções
        printf(". \n");
        wprintf(L"\nDigite 0 para parar o programa ou 1 para realizar outra operação: ");
        if (scanf(" %i", &continuar) != 1 || (continuar != 0 && continuar != 1)) {
            wprintf(L"\nEntrada inválida. ");
            system("pause");
            return 1;
